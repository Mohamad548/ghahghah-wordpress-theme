 ...
 * Usage: node run-lh.cjs <phase> <urls.json> <outDir>
 */
const fs = require('fs');
...
for (const page of pages) {
  ...
  const runs = [];
  for (let r = 1; r <= 3; r++) {
    const out = path.resolve(outDir, `${page}-mobile-run${r}.json`);
    console.log(`[${phase}] ${page} run ${r} -> ${out}`);
    const args = [
      '--yes',
      'lighthouse@12.2.1',
      url,
    ...
    const res = spawnSync('npx.cmd', args, {
      cwd: process.env.TEMP || 'C:\\Users\\Mohamad\\AppData\\Local\\Temp',
      encoding: 'utf8',
      shell: false,
      windowsHide: true,
    });
    if (res.status !== 0 || !fs.existsSync(out)) {
      runs.push({
        run: r,
        error: (res.stderr || res.stdout || `exit ${res.status}`).slice(0, 2000),
        status: res.status,
      });
      console.error('LH failed', res.status, (res.stderr || '').slice(0, 500));
      continue;
    }
    ...
    runs.push({
      ...
      images: targetImgs.filter((i) =>
        /pizza|parsley|corn-isolated|article-|bowl-of-real|corn-and-real|decorative-snack|packshot/i.test(
          i.url,
        ),
      ),
    });
